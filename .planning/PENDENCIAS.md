...
| PEND-027 | Infra / Backend deploy | Deployar `apps/api` (NestJS) em host real (Render/Railway/Fly/AWS) e setar `NEXT_PUBLIC_API_URL` na Vercel para o domínio público | Apenas `@nexo/web` está deployado; switcher de empresa e listagens dependem de `/api/empresas/*`. Fallback mock client-side ligado em `empresa-switcher.tsx` e `cadastros/empresas/page.tsx` para liberar testes na Vercel; remover quando o backend estiver online | Antes de qualquer teste/piloto com dados reais | P1 | Resolvida |
| PEND-028 | Infra / KMS real | Substituir `ALLOW_NO_KMS=1` por `KMS_CERT_KEY_ID` apontando para CMK AWS real em sa-east-1 | Render esta em us-east-1 com escape hatch ALLOW_NO_KMS=1 — uploads de certificado A1 falham. Apenas auth/CRUD/listagens funcionam. | Antes do primeiro upload de certificado real / homologacao SEFAZ | P1 | Aberta |
| PEND-029 | Infra / Migracao para AWS sa-east-1 | Migrar `apps/api` de Render para AWS Fargate sa-east-1 conforme [CLAUDE.md](../CLAUDE.md) | Render us-east-1 e setup operacional rapido para MVP. Producao definitiva exige AWS sa-east-1 com KMS CMK por tenant + S3 Object Lock + ElastiCache | Antes de cliente real / volume significativo | P2 | Aberta |
...
- `PEND-027` — `apps/api` deployado no Render (us-east-1) com Neon Postgres + Upstash Redis em 2026-05-03. `NEXT_PUBLIC_API_URL` apontando para a URL pública. Fallback mock client-side removido de `empresa-switcher.tsx` e `cadastros/empresas/page.tsx`. Hardening de KMS real e migração para AWS sa-east-1 movidos para `PEND-028`/`PEND-029`.

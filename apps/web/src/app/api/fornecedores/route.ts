/**
 * Route Handler proxy: /api/fornecedores (Plan 02-07 Task 2).
 *
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const data = await fetchApi(`/api/fornecedores${qs ? `?${qs}` : ''}`);
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const data = await fetchApi('/api/fornecedores', { method: 'POST', body });

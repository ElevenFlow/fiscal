/**
 * Route Handler proxy: /api/fornecedores/check-duplicate (Plan 02-07 Task 2 + Plan 02-03 CAD-09).
 *
 * Consultado em onBlur do campo CNPJ no form de fornecedor. Backend valida
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const data = await fetchApi(
      `/api/fornecedores/check-duplicate?cpfCnpj=${encodeURIComponent(sanitized)}`,
    );

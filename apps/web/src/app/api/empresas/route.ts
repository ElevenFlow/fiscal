/**
 * Route Handler proxy: /api/empresas (Plan 02-07 Task 2).
 *
 ...
 *
 * Existe também /api/empresas/minhas (Plan 01-07) — não conflita; rota mais
 * específica é resolvida primeiro pelo Next.
 */
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const data = await fetchApi(`/api/empresas${qs ? `?${qs}` : ''}`);
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const data = await fetchApi('/api/empresas', { method: 'POST', body });

/**
 * Route Handler proxy: /api/contabilidades/[id] (Plan 02-07 Task 2).
 *
...
export async function GET(_req: NextRequest, ctx: RouteContext): Promise<NextResponse> {
  ...
  try {
    const data = await fetchApi(`/api/contabilidades/${encodeURIComponent(id)}`);
...
export async function DELETE(_req: NextRequest, ctx: RouteContext): Promise<NextResponse> {
  const { id } = await ctx.params;
  try {
    await fetchApi(`/api/contabilidades/${encodeURIComponent(id)}`, { method: 'DELETE' });
